...
ang_bet  = lambda a, b: torch.dot(a,b)
distances = []
displacements = []
ending_simils = []
loss_graph = []
...
for sample in range(8):
    for perm in range(250):
        ...
        print([torch.abs(ang_bet(order[i], order[i+1])) for i in range(7)])
...
axs[0].scatter(distances, loss_graph, s = 0.1)
axs[1].scatter(ending_simils, loss_graph, s = 0.1)
plt.savefig("pn.png")
